...
public typealias PingComplete = ((_ response: DDPingResponse?, _ error: Error?) -> Void)

public enum DDPingError: Error, Equatable {
    case requestError   //发起失败
    case receiveError   //响应失败
    case timeout        //超时
}

public struct DDPingResponse {
    public var pingAddressIP = ""
    public var responseTime: DDPingTimeInterval = .second(0)
...
public enum DDPingTimeInterval {
...
@objcMembers open class DDPingTools: NSObject {
    ...
    private var pingInterval: DDPingTimeInterval = .second(0)
    private var complete: PingComplete?
    private var lastSendItem: DDPingItem?
    private var lastReciveItem: DDPingItem?
    ...
    ///   - interval: 是否重复定时ping
    ///   - complete: 请求的回调
    public func start(pingType: SimplePingAddressStyle = .any, interval: DDPingTimeInterval = .second(0), complete: PingComplete? = nil) {
        //移除消息订阅
        NotificationCenter.default.removeObserver(self)
        //切到后台
    ...
    public func stop() {
        ...
        if self.showNetworkActivityIndicator == .auto || self.showNetworkActivityIndicator == .none {
            DDPingNetworkActivityIndicator.shared.isHidden = true
        } else {
            DDPingNetworkActivityIndicator.shared.isHidden = false
        }
        //结束状态
        self._stop()
    }
...
private extension DDPingTools {
    /// 开始ping请求
    func _start(pingType: SimplePingAddressStyle = .any, interval: DDPingTimeInterval = .second(0), complete: PingComplete? = nil) {
        self._stop()
...
extension DDPingTools: SimplePingDelegate {
    ...
    public func simplePing(_ pinger: SimplePing, didFailWithError error: Error) {
        ...
        DDPingNetworkActivityIndicator.shared.update(time: 460)
        if let complete = self.complete {
            complete(nil, DDPingError.requestError)
        }
        //标记完成
        self._complete()
        //停止ping
    ...
    public func simplePing(_ pinger: SimplePing, didSendPacket packet: Data, sequenceNumber: UInt16) {
        ...
        self.isPing = true
        lastSendItem = DDPingItem(sendTime: Date(), sequence: sequenceNumber)
        //发送数据之后监测是否超时
        if timeout.second > 0 {
            checkTimer?.invalidate()
            checkTimer = nil
            checkTimer = Timer(timeInterval: timeout.second, repeats: false, block: { [weak self] (_) in
                ...
                if self.lastSendItem?.sequence != self.lastReciveItem?.sequence {
                    ...
                    //超时
                    if let complete = self.complete {
                        complete(nil, DDPingError.timeout)
                    }
                    //标记完成
                    self._complete()
                    //停止ping
    ...
    public func simplePing(_ pinger: SimplePing, didFailToSendPacket packet: Data, sequenceNumber: UInt16, error: Error) {
        ...
        DDPingNetworkActivityIndicator.shared.update(time: 460)
        if let complete = self.complete {
            complete(nil, DDPingError.receiveError)
        }
        //标记完成
        self._complete()
        //停止ping
    ...
    public func simplePing(_ pinger: SimplePing, didReceivePingResponsePacket packet: Data, sequenceNumber: UInt16) {
        if let sendPingItem = lastSendItem {
            ...
            DDPingNetworkActivityIndicator.shared.update(time: Int(time))
            if let complete = self.complete {
                let response = DDPingResponse(pingAddressIP: pingAddressIP, responseTime: .millisecond(time), responseBytes: packet.count)
                complete(response, nil)
            }
            lastReciveItem = DDPingItem(sendTime: Date(), sequence: sequenceNumber)
            //标记完成
            self._complete()
        }
    }
